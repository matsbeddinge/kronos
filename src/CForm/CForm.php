...
class CFormElement implements ArrayAccess{
  ...
  public function __construct($name, $attributes=array()) {
    ...
    $this['name'] = $name;
    if(is_callable('CKronos::Instance()')) {
      $this->characterEncoding = CKronos::Instance()->config['character_encoding'];
    } else {
  ...
  public function GetHTML() {
    ...
    if($type && $this['type'] == 'submit') {
      ...
      return "<p><label for='$id'>$label</label><br><textarea id='$id'{$type}{$class}{$name}{$autofocus}{$readonly}>{$onlyValue}</textarea>{$messages}</p>\n";
    } else if($type && $this['type'] == 'hidden') {
      return "<input id='$id'{$type}{$class}{$name}{$value} />\n";
    } else if($type && $this['type'] == 'checkbox') {
	  return "<p><input id='$id'{$type}{$class}{$nameArray}{$checked}{$value} /><label for='$id'>$label</label></p>\n";
	} else {
