...
class CMComment extends CObject implements IHasSQL, ArrayAccess, IModule {
  ...
  public static function SQL($key=null, $args=null) {
    ...
    $queries = array(
      ...
      'select * by content' => "SELECT Comment.*, User.acronym as owner FROM Comment INNER JOIN User ON Comment.idUser=User.id WHERE idContent=? AND Comment.deleted IS NULL;",
  ...
  public function Delete($id) {
    ...
    $rowcount = $this->db->RowCount();
    if($rowcount) {
      $this->AddMessage('success', "Successfully set comment with id:" . $id . " as deleted.");
    } else {
      $this->AddMessage('error', "Failed to set comment with id:" . $id . " as deleted.");
    }
    return $rowcount === 1;
  }
